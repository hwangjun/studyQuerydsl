...
//        member.username.startsWith("member") // like 'member%' 검색
    ...
    public void join_on_filtering() {
        ...
        for (Tuple tuple : result) {
            System.out.println("truple = " + tuple);
        }
    }

    @Test
    public void fetchJoinUnUse() {
        em.flush();
        em.clear();
    ...
    void concat() {
        List<String> result = queryFactory
                .select(
                        member.username.concat("_").concat(member.age.stringValue()) //ENUM 처리할때도 stringValue() 사용
                )
                .from(member)
                .where(member.username.eq("member1"))
                .fetch();
